...
# <---- helper functions ---->
def long_to_pycombat_df(protein_df: pd.DataFrame, value_name: str | None = None) -> pd.DataFrame:
    """
...
def get_batches_for_each_sample_in_order(transformed_protein_df: pd.DataFrame, metadata_df: pd.DataFrame):
    samples_in_order = transformed_protein_df.columns
    batches_in_order = []
    for sample in samples_in_order:
        batches_in_order.append(metadata_df[metadata_df["Sample"] == sample]["Batch"].iloc[0])
...
def combat_correction(protein_df: pd.DataFrame, metadata_df: pd.DataFrame):
    ...
    batches_in_order = get_batches_for_each_sample_in_order(transformed_protein_df=transformed_protein_df, metadata_df=metadata_df)
    batch_corrected_protein_df = pycombat_norm(transformed_protein_df, batches_in_order)
    batch_corrected_protein_df = pycombat_df_to_long(batch_corrected_protein_df, protein_df)
